...
//		forEach(System.out::println);
		...
		List<String> result = list.stream().skip(100).filter(i -> i % 2 == 1).sorted((i1, i2) -> i2 - i1).distinct()
	...
	private static List<Integer> generateIntegerList(Integer seed, Integer size) {
		return Stream.generate(() -> new Random().nextInt(seed)).limit(size).map(i -> i + 500).toList();
	}
}
